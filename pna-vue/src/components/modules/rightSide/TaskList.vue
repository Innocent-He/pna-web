<template>
  ...
  >
    <a-list :data-source="tasks" :locale="{emptyText:'当前没有执行中的任务'}">
      <a-list-item slot="renderItem" slot-scope="{ id,result,algName,status,ownerName,createTime }" key="id">
        ...
        <a-button v-else-if="isOwner(ownerName)&&status===2&&algName==='generate'" type="link"
                  @click="generateNet(result)">Generate
        </a-button>
        <a-button v-else-if="isOwner(ownerName)&&status===2" type="link" @click="download(result)">Download</a-button>
        <a-button v-if="isOwner(ownerName)&&(status===2||status===3)" type="link" @click="deleteTask(id)">Delete
        </a-button>
      </a-list-item>
...
export default {
  ...
  watch: {
    tasks: {
      handler(list) {
        this.taskReq.pageNo = Math.floor(list.length / this.taskReq.pageSize) + 1;
      },
      deep: true
    }
  }
